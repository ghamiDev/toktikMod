...
def random_flip_segments(segments, progress, start, end):
    total = len(segments)
    if total <= 2:
        return segments  # tidak ada middle segment

    first_seg = segments[0]
    last_seg = segments[-1]
    middle_segs = segments[1:-1]

    flip_count = len(middle_segs) // 2
    flip_targets = random.sample(middle_segs, flip_count) if flip_count > 0 else []
    ...
    for seg in middle_segs:
        if seg in flip_targets:
            ...
            flipped_middle.append(out)
        else:
            flipped_middle.append(seg)
    ...
    return [first_seg] + flipped_middle + [last_seg]
...
def apply_effect(input_path, output_path, mute_final=False):
    cmd = [
        ...
        "-vf",
        (
            # pastikan tinggi min 1920, lebar mengikuti rasio
            "scale=-2:1920,"
            # crop CENTER ke 9:16 tanpa melebihi frame
            "crop=1080:1920:(iw-1080)/2:(ih-1920)/2,"
            # efek ringan (aman untuk video buram)
            "unsharp=3:3:0.4,"
            "eq=saturation=1.04:contrast=1.02:brightness=0.01"
        ),
    ]
    ...
    cmd += [
        "-c:v", "libx264",
        "-preset", "medium",
        "-pix_fmt", "yuv420p",  # PENTING: biar bisa diputar di semua device
        output_path
    ]
    ...
    if st.button("Proses Video"):
        ...
        for i, norm_path in enumerate(normalized_paths):
            ...
            for seg in segments:
                ...
                try:
                    ...
                    if dur >= 3:
                        ...
                        # hapus segmen pendek agar tidak kepakai
                        try:
                            os.remove(seg)
                        except:
                            pass
            ...
            if len(valid_segments) == 0:
                st.error(f"Tidak ada segmen ≥ 3 detik pada file ke-{i+1}. File dilewati.")
                continue
            ...
            # ================= SHUFFLE SEGMENT TENGAH SAJA =================
            if len(valid_segments) > 2:
                first_seg = valid_segments[0]
                last_seg = valid_segments[-1]
                middle_segs = valid_segments[1:-1]

                random.shuffle(middle_segs)

                valid_segments = [first_seg] + middle_segs + [last_seg]
            # jika segmen <= 2, biarkan urutan asli (tidak diacak)
