...
def human_delay(min_s=0.15, max_s=0.45):
...
EDIT_PROFILES = {
    "affiliate_soft_A": {
        "segment_time": (2.8, 3.4),
        "flip_prob": 0.15,
        "crop_offset": 2,
        "shuffle_mode": "local",
        "speed_range": (0.99, 1.015),
    },
    "affiliate_soft_B": {
        ...
        "crop_offset": 3,
        "shuffle_mode": "local",
        "speed_range": (0.985, 1.02),
    },
    "mass_hard": {
        "segment_time": (2.0, 2.6),
        "flip_prob": 0.6,
        "crop_offset": 5,
...
# optimasi dengan opencv - semi ML
def auto_hook_checker(video_path, check_duration=2.5):
    cap = cv2.VideoCapture(video_path)
    fps = cap.get(cv2.CAP_PROP_FPS)
    max_frames = int(fps * check_duration)
    ...
    if len(motion_scores) == 0:
        return {
            "status": "FAIL",
            "reason": "Video terlalu statis di awal"
        }

    motion_avg = np.mean(motion_scores)
    bright_var = np.std(brightness)

    # Threshold aman (hasil lapangan, bukan teori)
    if motion_avg < 3.5 and bright_var < 4:
        return {
            "status": "WEAK",
            "reason": "Hook awal terlalu halus / minim perubahan"
        }

    return {
        "status": "OK",
        "reason": "Hook awal memiliki motion & perubahan visual"
    }


def auto_select_mode(account_age_days, force_mode=None):
    if force_mode:
        return force_mode
...
def random_crop_offset_segment(input_path, output_path, max_offset=5):
    """
    Apply small random crop offset (±2–5 px) per segment.
    No zoom, no resize, safe for quality.
    """
    # random offset (hindari 0 terus)
    ox = random.randint(2, max_offset) * random.choice([-1, 1])
    oy = random.randint(2, max_offset) * random.choice([-1, 1])
...
def random_flip_segments_prob(segments, flip_prob=0.4):
    out = []
    for seg in segments:
        if random.random() < flip_prob:
            flipped = os.path.join(
                TEMP_DIR,
                "flip_" + os.path.basename(seg)
            )
            run([
                "ffmpeg", "-y",
                "-i", seg,
...
def local_shuffle(segments, window=3):
    out = []
...
def neighbor_swap(segments, prob=0.4):
    out = segments[:]
...
def apply_speed_variation(input_path, output_path, speed_range=(0.98, 1.02)):
...
# ============================== MAIN PROCESS =================================
st.subheader("⚙️ Profil Akun TikTok")
account_age = st.number_input(
    "Umur akun TikTok (hari)",
    min_value=0,
    max_value=3650,
    value=30
)
...
force_mode = st.selectbox(
    "Force Mode (opsional)",
    ["Auto", "Affiliate (Soft)", "Mass Upload (Hard)"]
)

force_map = {
    "Affiliate (Soft)": "affiliate_soft",
    "Mass Upload (Hard)": "mass_hard"
}

selected_mode = auto_select_mode(
    account_age_days=account_age,
    force_mode=force_map.get(force_mode)
)

profile = EDIT_PROFILES[selected_mode]

st.success(f"Mode aktif: {selected_mode.upper()}")
...
if len(uploaded_files) > 0:
    if st.button("Proses Video"):
        ...
        render_seed = init_render_seed()
        st.info(f"Render Seed: {render_seed}")
        ...
        for i, norm_path in enumerate(normalized_paths):
            ...
            # segments = split_reencode(norm_path, prefix=prefix) v app.py

            seg_time = random.uniform(*profile["segment_time"])
            segments = split_reencode(norm_path, prefix=prefix, segment_time=seg_time)
            human_delay()
            ...
            # valid_segments = random_flip_segments(valid_segments, progress, pct, pct+20) - v app.py
            # === RANDOM CROP OFFSET PER SEGMENT (NEW) ===
            offset_segments = []

            for seg in valid_segments:
                out = os.path.join(
                    TEMP_DIR,
                    "crop_" + os.path.basename(seg)
                )
                # random_crop_offset_segment(seg, out, max_offset=5) v app.py
                random_crop_offset_segment(
                    seg,
                    out,
                    max_offset=profile["crop_offset"]
                )

                offset_segments.append(out)

            valid_segments = offset_segments
            human_delay()
            # === RANDOM FLIP (MODE AWARE) ===
            valid_segments = random_flip_segments_prob(
                valid_segments,
                flip_prob=profile["flip_prob"]
            )
            ...
            # shuffle segments
            # random.shuffle(valid_segments) - v app.py
            if profile["shuffle_mode"] == "local":
                valid_segments = local_shuffle(valid_segments, window=3)
            else:
                valid_segments = neighbor_swap(valid_segments, prob=0.4)
            ...
            # processed_outputs.append(processed_out) - v app.py
            ...
            # === MICRO SPEED VARIATION (MODE AWARE) ===
            speed_out = f"{TEMP_DIR}/processed_{i}_speed.mp4"
            ...
            processed_outputs.append(speed_out)
            human_delay()
        ...
        # ===================== AUTO HOOK CHECKER =====================
        st.subheader("🎯 Auto Hook Checker")
        hook_result = auto_hook_checker(out_path)

        if hook_result["status"] == "OK":
            st.success("HOOK AMAN ✅ — Video layak upload")
        elif hook_result["status"] == "WEAK":
            st.warning(f"HOOK LEMAH ⚠️ — {hook_result['reason']}")
        else:
            st.error(f"HOOK GAGAL ❌ — {hook_result['reason']}")
